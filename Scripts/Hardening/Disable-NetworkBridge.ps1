...
<#
.SYNOPSIS
    Disables network bridging on domain networks.

.DESCRIPTION
    Sets NC_AllowNetBridge_NLA to 0 in the Network Connections policy registry key,
    which prevents users from creating network bridges between adapters.
...
# ── Constants ──────────────────────────────────────────────
$SCRIPT_NAME    = "Disable-NetworkBridge"
$SCRIPT_VERSION = "1.0"
