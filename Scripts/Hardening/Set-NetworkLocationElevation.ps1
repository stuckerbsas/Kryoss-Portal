...
<#
.SYNOPSIS
    Requires elevation when domain users set a network location.

.DESCRIPTION
    Sets the NC_StdDomainUserSetLocation registry value to require domain users
    to elevate (UAC prompt) when changing a network location. This prevents
    standard users from reclassifying network profiles without administrator approval.
...
.EXAMPLE
    .\Set-NetworkLocationElevation.ps1
    Applies the hardening setting silently (NinjaRMM/Intune deployment).

.EXAMPLE
    .\Set-NetworkLocationElevation.ps1 -WhatIf
...
# ── Constants ──────────────────────────────────────────────
$SCRIPT_NAME    = "Set-NetworkLocationElevation"
$SCRIPT_VERSION = "1.0"
