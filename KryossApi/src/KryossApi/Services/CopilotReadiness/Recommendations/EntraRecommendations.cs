...
/// <summary>
/// Generates all Entra ID recommendations from pre-computed <see cref="EntraInsights"/>.
/// Ports 14 Python recommendation modules:
///   AAD_PREMIUM (P1), AAD_PREMIUM_P1, AAD_PREMIUM_P2, AAD_PREMIUM_CONDITIONAL_ACCESS,
///   AAD_PREMIUM_IDENTITY_PROTECTION, INTUNE_A, MFA_PREMIUM, AAD_GOVERNANCE,
///   ENTRA_INTERNET_ACCESS, ENTRA_INTERNET_ACCESS_FRONTLINE, ENTRA_PRIVATE_ACCESS,
///   ENTRA_PRIVATE_ACCESS_CA
///
/// Each Generate* method returns 1-N RecommendationResult items, matching the
/// multi-observation pattern from the Python source.
/// </summary>
public static class EntraRecommendations
{
    ...
    public static List<RecommendationResult> Generate(EntraInsights ins)
    {
        var all = new List<RecommendationResult>();

        all.AddRange(GenerateAadPremiumP1(ins));       // CA, MFA, legacy auth, passwordless, group licensing
        all.AddRange(GenerateAadPremiumP2(ins));       // PIM, access reviews, risky users, B2B, consent, risky apps
        all.AddRange(GenerateConditionalAccess(ins));  // CA policy coverage + Copilot targeting
        all.AddRange(GenerateIdentityProtection(ins)); // Risky users, risk-based CA
        all.AddRange(GenerateIntune(ins));             // Device compliance + CA integration
        all.Add(GenerateMfaPremium(ins));              // MFA license-level only
        all.AddRange(GenerateGovernance(ins));         // PIM + access reviews + consent + risky apps
        all.AddRange(GenerateInternetAccess(ins));     // GSA web filtering + traffic forwarding
        all.Add(GenerateInternetAccessFrontline(ins)); // Frontline secure access
        all.AddRange(GeneratePrivateAccess(ins));      // Private Access connectors
        all.Add(GeneratePrivateAccessCa(ins));         // CA for Private Access
    ...
    private static List<RecommendationResult> GenerateAadPremiumP1(EntraInsights ins)
    {
    ...
    private static List<RecommendationResult> GenerateAadPremiumP2(EntraInsights ins)
    {
    ...
    private static List<RecommendationResult> GenerateConditionalAccess(EntraInsights ins)
    {
    ...
    private static List<RecommendationResult> GenerateIdentityProtection(EntraInsights ins)
    {
    ...
    private static List<RecommendationResult> GenerateIntune(EntraInsights ins)
    {
    ...
    private static RecommendationResult GenerateMfaPremium(EntraInsights ins)
    {
    ...
    private static List<RecommendationResult> GenerateGovernance(EntraInsights ins)
    {
    ...
    private static List<RecommendationResult> GenerateInternetAccess(EntraInsights ins)
    {
    ...
    private static RecommendationResult GenerateInternetAccessFrontline(EntraInsights ins)
    {
    ...
    private static List<RecommendationResult> GeneratePrivateAccess(EntraInsights ins)
    {
    ...
    private static RecommendationResult GeneratePrivateAccessCa(EntraInsights ins)
    {
        if (ins.PrivateAccessStatus == "Success")
        {
            return RecommendationResult.Success(Svc,
