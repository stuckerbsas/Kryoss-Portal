...
public class ReportService : IReportService
{
    ...
    public async Task<string> GenerateHtmlReportAsync(Guid runId, string reportType = "technical", string? frameworkCode = null, string lang = "en")
    {
        ...
        // Per-run reports also get the enriched signoff footer with the
        // operator's name / email / phone — baseline consistency across
        // all 6 report types.
        var userInfo = await BuildReportUserInfoAsync(run.Organization);

        return reportType switch
        {
            "executive" => BuildExecutiveReport(run, results, branding, frameworkName, frameworkScores, enrichment, userInfo),
            "presales" => BuildPresalesReport(run, results, branding, frameworkName, frameworkScores, enrichment, userInfo),
            _ => BuildTechnicalReport(run, results, branding, frameworkName, frameworkScores, enrichment, userInfo)
        };
    }
    ...
    private static string BuildTechnicalReport(AssessmentRun run, List<ReportControlResult> results,
        ...
        MachineEnrichment enrichment, ReportUserInfo userInfo)
    {
        ...
        AppendHtmlHead(sb, $"{reportTitle} - {run.Machine.Hostname}", brand, isOrgReport: false,
            user: userInfo, detail: $"Run {run.Id} · {run.Machine.Hostname}");
